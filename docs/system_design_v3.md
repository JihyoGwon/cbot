...
## 251120 문제점
1. (긴급) 상담사 발화 문제
    - 지적과 판단. 예시: 1년 넘게 알았는데도 계속 참고 있다니, 네 속이 정말 말이 아니겠네. 많이 지치고 답답하겠다. 그 친구가 네 말을 잘 못 알아듣는 이유가 뭐라고 생각해? 혹시 네가 말하는 방식 때문이라고 생각한 적은 없어? 아니면 친구가 원래 다른 사람 말을 잘 못 듣는 편인 것 같아?
2. task 완료 판단 일관성 문제

## 시스템 아키텍처 개선 필요 사항 (비판적 검토)

### High Priority (긴급)

#### 1. 캐시 일관성 문제
**문제점**:
- 비동기 작업(`_check_part_transition_async`, `_check_part2_task_update_async`)이 캐시를 업데이트하지만, 동기 실행과의 동기화가 보장되지 않음
- 동일 세션에 동시 요청 시 캐시 불일치 가능
- 캐시 무효화 전략이 명확하지 않음

**영향**: 
- Part 전환, Task 업데이트가 반영되지 않을 수 있음
- 사용자에게 잘못된 상태 표시 가능

**해결 방안**:
- 캐시 무효화 전략 수립 (TTL 또는 이벤트 기반)
- 분산 락 또는 트랜잭션 메커니즘 도입
- 캐시 업데이트 시 Firestore와 동기화 보장

#### 2. Race Condition 위험
**문제점**:
- 여러 비동기 작업이 동시에 Firestore를 업데이트할 수 있음
- 예: `_check_part_transition_async`와 `_check_part2_task_update_async`가 동시 실행 시 충돌 가능
- `part2_task_update_count` 업데이트와 Task 업데이트가 경쟁 상태 발생 가능

**영향**:
- 데이터 손실 또는 불일치
- Task 업데이트 횟수 제한이 제대로 작동하지 않을 수 있음

**해결 방안**:
- Firestore 트랜잭션 사용
- 비동기 작업 간 순서 보장 메커니즘
- 업데이트 전 조건 재확인 (Optimistic Locking)

#### 3. Part 전환 로직의 중복
**문제점**:
- 동기 실행(315-416줄)과 비동기 실행(612-668줄)에서 모두 Part 전환 로직이 있음
- 비동기에서 전환이 발생하면 다음 요청에서 동기 로직이 다시 실행될 수 있음
- 단일 진실 공급원(Single Source of Truth) 부재
...
**해결 방안**:
- Part 전환 로직을 단일 함수로 통합
- 전환 상태를 명확히 추적 (예: `transitioning` 플래그)
- 비동기 전환 완료 후 동기 로직 스킵
...
**문제점**:
```python
# 177줄: 매번 Firestore에서 읽음
current_part = self.part_manager.get_current_part(conversation_id)
```
- `get_current_part`가 매번 Firestore를 읽어 캐시 이점이 없음
- 세션 캐시에 이미 `current_part` 정보가 있음에도 불구하고 재조회
...
**해결 방안**:
- 캐시 우선 사용, 필요 시에만 Firestore 읽기
- `get_current_part`에 캐시 파라미터 추가

#### 5. Part 2→3 전환 조건이 단순함
**문제점**:
```python
# part_manager_service.py 62-71줄
# 주석: "실제로는 LLM이 판단해야 함"
all_sufficient = all(t.get('status') in ['sufficient', 'completed'] for t in current_part_tasks)
```
- Part 2는 동적 Task 생성인데, 전환 조건이 단순 상태 체크만 수행
- 설계 문서에는 "주요 Task들이 충분히 다뤄짐 (LLM 판단)"이라고 명시되어 있으나 구현되지 않음

**영향**:
- Part 2에서 중요한 Task를 놓치고 Part 3로 넘어갈 수 있음
- 상담 품질 저하 가능

**해결 방안**:
- LLM 기반 Part 2 완료 판단 로직 구현
- 또는 Task 중요도(priority) 기반 가중치 계산

#### 6. 에러 처리 부족
**문제점**:
- 비동기 작업의 에러가 로그에만 남고 사용자에게 전달되지 않음
- Task 업데이트 실패, Part 전환 실패 시 사용자가 알 수 없음
- 프론트엔드에 에러 상태 전달 메커니즘 없음

**영향**:
- 사용자는 시스템 오류를 인지하지 못함
- 디버깅 어려움

**해결 방안**:
- 세션에 `error_log` 필드 추가
- 프론트엔드에 에러 상태 표시
- 비동기 작업 실패 시 재시도 메커니즘

### Low Priority (개선)

#### 7. Task 업데이트 제한 로직의 위치
**문제점**:
- `part2_task_update_count` 체크가 비동기 함수에만 있음
- 동기 실행 경로에서는 체크되지 않음 (현재는 비동기만 호출되지만 일관성 문제)

**해결 방안**:
- 공통 검증 로직으로 분리
- `update_part2_tasks` 호출 전에 항상 체크

#### 8. 메시지 중복 체크 로직의 한계
**문제점**:
```python
# 501-508줄: 마지막 메시지만 체크
is_last_user_msg = (
...
```
- `conversation_history`에 이미 현재 메시지가 포함되어 있을 수 있음
- 정확한 중복 방지가 어려움

**해결 방안**:
- `conversation_history`에 현재 메시지 포함 여부 확인 로직 강화
- 또는 `conversation_history`를 전달할 때 현재 메시지 제외

#### 9. ThreadPoolExecutor 리소스 관리
**문제점**:
- `ThreadPoolExecutor`가 인스턴스 변수로만 존재하고 명시적 종료가 없음
- 장기 실행 시 리소스 누수 가능

**해결 방안**:
- 컨텍스트 매니저 사용 또는 명시적 shutdown
- 애플리케이션 종료 시 정리 로직 추가

#### 10. 세션 캐시 무효화 전략 부재
**문제점**:
- 캐시가 무효화되는 시점이 명확하지 않음
- `force_refresh` 파라미터가 있지만 사용처가 제한적
- 메모리 누수 가능 (오래된 세션 캐시)

**해결 방안**:
- TTL(Time To Live) 기반 자동 무효화
...
### 문제 예시
```
"1년 넘게 알았는데도 계속 참고 있다니, 네 속이 정말 말이 아니겠네. 
많이 지치고 답답하겠다. 그 친구가 네 말을 잘 못 알아듣는 이유가 뭐라고 생각해? 
혹시 네가 말하는 방식 때문이라고 생각한 적은 없어?"
```

**문제점**:
- 사용자 탓으로 돌리기 ("네가 말하는 방식 때문")
- 판단과 지적 ("네 속이 정말 말이 아니겠네")
- 원인 추궁 ("이유가 뭐라고 생각해?")
- 정보 수집 단계에서 조언/해결책 제시

### 원인 분석

#### 1. 시스템 프롬프트의 충돌
**문제**:
- "구체적 조언 제공" (config.py 32줄) vs "비판 금지" (35줄)
- "해결 방안을 제시" (34줄)가 정보 수집 단계에서도 조언을 유도
- Part별 제약사항이 없어 모든 Part에서 동일한 지시사항 적용

**영향**: 
- 정보 수집 단계(Part 1, 2)에서도 조언/해결책을 제시하려고 함
- "구체적 조언" 지시사항이 "비판 금지"보다 강하게 작용

#### 2. Task의 `restrictions`가 프롬프트에 반영되지 않음
**문제**:
- Task에 `restrictions` 필드가 있어도 Counselor 프롬프트에 포함되지 않음
- "해결책 제시 금지", "조언 금지" 같은 제약이 전달되지 않음

**코드 위치**: `counselor_service.py` 125-127줄
```python
...
#### 3. Part별 제약사항 부재
**문제**:
- Part 정보가 "현재 Part 2 진행 중입니다"만 있고, Part별 목적과 제약이 없음
- Part 2는 "탐색/정보 수집" 단계인데, "조언 금지" 제약이 명시되지 않음

**코드 위치**: `counselor_service.py` 105줄
```python
part_info = f"\n현재 Part {current_part} 진행 중입니다.\n"
# Part별 목적과 제약이 없음!
```

#### 4. Module 가이드라인과 시스템 프롬프트의 충돌
**문제**:
- "질문 기법" Module이 선택되면 질문 중심으로 가려 하지만
- 시스템 프롬프트의 "구체적 조언"과 충돌
- Module 가이드라인이 시스템 프롬프트보다 약하게 작용
...
1. **Part별 프롬프트 분리**
   - Part 1, 2: "정보 수집 단계 - 조언/해결책 제시 금지, 공감과 질문에 집중"
   - Part 3: "마무리 단계 - 목표 설정과 다음 단계 안내"

2. **Task의 `restrictions`를 프롬프트에 포함**
   ```python
...
3. **시스템 프롬프트 수정**
   - "구체적 조언"을 Part별로 조건부로 변경
   - 정보 수집 단계에서는 "조언 금지" 명시
   - "비판 금지"를 더 강조하고 구체화

4. **Part별 제약사항 명시**
   ```python
   part_constraints = {
       1: "정보 수집 단계입니다. 조언이나 해결책을 제시하지 말고, 공감과 질문에 집중하세요.",
       2: "탐색 단계입니다. 사용자의 감정과 경험을 깊이 이해하는 데 집중하세요. 조언은 하지 마세요.",
       3: "마무리 단계입니다. 상담 내용을 요약하고, 목표를 설정하며, 다음 단계를 안내하세요."
   }
   ```
...
5. **Module 가이드라인 강화**
   - Module 선택 시 시스템 프롬프트와의 충돌 방지
   - Module별 제약사항 명시

6. **Supervision 피드백 강화**
   - "지적/판단" 감지 시 즉시 피드백
   - "조언 제시" 감지 시 Part별 제약 위반 알림

## 우선순위별 개선 로드맵

### Phase 0 (긴급 - LLM 발화 품질)
1. ⚠️ Part별 프롬프트 분리
2. ⚠️ Task의 `restrictions`를 프롬프트에 포함
3. ⚠️ 시스템 프롬프트 수정 (Part별 조건부 조언)
4. ⚠️ Part별 제약사항 명시
...
4. ✅ Part 2→3 전환 조건 개선
5. ✅ 에러 처리 강화
6. ✅ 불필요한 Firestore 읽기 최소화

### Phase 3 (중장기 개선)
7. ✅ Task 업데이트 제한 로직 리팩토링
8. ✅ 세션 캐시 무효화 전략
9. ✅ ThreadPoolExecutor 리소스 관리
10. ✅ 메시지 중복 체크 로직 개선