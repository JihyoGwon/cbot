...
**구현**:
- **상담 시작 전 사용자가 직접 선택** (현재 구현 방식)
- 상담 페이지에서 페르소나 타입과 상담 레벨을 선택한 후 "새 대화 시작" 버튼 클릭
- 선택된 페르소나 정보는 세션 생성 시 `user_persona` 필드로 저장
- Firestore에 `user_persona.type`, `user_persona.type_specific_keywords`, `user_persona.common_keywords`, `user_persona.counseling_level` 저장
- 타입별 특성은 Admin 페이지에서 관리 (PersonaService)
...
**구현**:
- 사용자가 페르소나 타입을 선택하면 해당 타입의 키워드가 자동으로 할당됨
- Firestore에 `user_persona.type_specific_keywords` (타입 특화 키워드 4개)와 `user_persona.common_keywords` (공통 키워드 4개) 저장
...
- **상담 시작 전 사용자가 직접 선택** (현재 구현 방식)
- 초기 상담은 레벨 1 권장
- 상담 진행에 따라 레벨 업데이트 가능 (향후 기능)

**구현**:
- 상담 페이지에서 사용자가 레벨 1~5 중 선택
...
**페르소나 정보**:
- 페르소나 타입과 상담 레벨은 **상담 시작 전 사용자가 직접 선택**
- 선택된 페르소나 정보는 세션 생성 시 `user_persona` 필드로 저장됨
- Part 1 대화 중에는 페르소나 정보를 참고하여 개인화된 상담 진행 가능
...
1. **페르소나 정보 조회**:
   - 세션에 저장된 `user_persona` 정보 조회
   - 타입, 타입별 특화 키워드, 공통 키워드, 상담 레벨 확인
...
5. **Part 2 목표 생성**:
   - 페르소나 타입 + 선택된 키워드 + 레벨 + Part 1 대화 내용을 바탕으로
...
Task Planner: Part 2 목표 수립 시작
    ↓
1. 페르소나 정보 조회 (세션에서)
   → user_persona.type (사용자가 선택한 타입)
   → user_persona.type_specific_keywords (타입별 특화 키워드 4개)
   → user_persona.common_keywords (공통 키워드 4개)
   → user_persona.counseling_level (사용자가 선택한 레벨 1~5)
    ↓
2. Part 1 대화 분석
   → 전체 대화 내용 분석
   → 사용자 문제, 감정, 상황 파악
...
4. 레벨 기반 범위 결정
   → 사용자가 선택한 상담 레벨에 따라:
   → 레벨 1: 표면적 탐색 (라포 형성, 문제 인식)
   → 레벨 3: 깊은 탐색 (패턴 인식, 원인 파악)
   → 레벨 5: 해결 중심 (문제 해결, 유지)
    ↓
5. Part 2 목표 생성 (LLM)
   → 페르소나 타입 + 선택된 키워드 + 레벨 + 대화 내용 기반
   → 구체적이고 측정 가능한 목표 생성
...
**주요 기능**:
- 페르소나 타입 관리 (16타입 CRUD)
- 상담 키워드 관리 (타입 특화 4개 + 공통 4개)
- 상담 레벨 관리 (1~5 단계 정의)
- 페르소나 정보 조회/업데이트
- **Admin 페이지 제공**: 페르소나 타입, 공통 키워드, 상담 레벨 관리

**페르소나 선택**:
- 상담 시작 전 사용자가 상담 페이지에서 직접 선택
- 선택된 페르소나 정보는 세션 생성 시 `user_persona` 필드로 저장
